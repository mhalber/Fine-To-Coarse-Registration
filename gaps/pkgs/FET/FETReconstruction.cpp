  ...
  const R3Affine& transformation21, RNScalar w)
{
  ...
  R3Point position1, position2;

  position2 = shape2->Origin();
  position2[0] += r; 
  position1 = position2; position1.Transform(transformation21);  
  AddPointPointCorrespondenceEquations(system, shape1, shape2, position1, position2, w / 3.0);
   ...
   FETShape *shape1, FETShape *shape2)
{
  ...
  printf("  M %d %d : %g %d %d\n", shape1->reconstruction_index, 
                                   shape2->reconstruction_index, 
                                   best_score, 
                                   shape1->NFeatures(), 
                                   shape2->NFeatures() );
