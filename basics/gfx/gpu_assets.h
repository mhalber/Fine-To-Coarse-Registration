...
enum GeometryPropertiesFlags_
{
  ...
  USER_DATA = 1 << 8, // layout( location = 8 )
  STRUCTURED = 1 << 9,
...
u64 _get_offset( const bsc::gpu_geometry * geo, const i32 flag )
{
  ...
  if ( geo->flags & USER_DATA )
  {
    if ( flag & USER_DATA ) return offset * geo->n_indices;
    offset += sizeof(r32); 
  }
                ...
                const i32 flags )
{
  ...
  if ( flags & USER_DATA )
  {
    u64 offset = _get_offset( geo, flags & USER_DATA);
    u64 current_size = sizeof(r32) * geo->n_indices;
    glBufferSubData( GL_ARRAY_BUFFER, offset, 
                                      current_size, 
                                      &(host_data->user_data[0]) );
               ...
               const i32 flags )
{
  ...
  glGenVertexArrays( 1, (GLuint*)&(geo->vao)  );
  glGenBuffers( 1, (GLuint*)&(geo->vbo) );
  if ( flags & STRUCTURED )
  {
    glGenBuffers( 1, (GLuint*)&(geo->ebo) );
  }
  ...
  if ( flags & USER_DATA ) buf_size += sizeof(r32);
...
free_gpu_geo( gpu_geometry * geo )
{
  ...
  {
    glDeleteBuffers(1, (GLuint*)&(geo->ebo) );
  }
  glDeleteVertexArrays( 1, (GLuint*)&(geo->vao) );
  geo->vao = -1;
  geo->vbo = -1;
  geo->ebo = -1;
  geo->n_indices  = -1;
