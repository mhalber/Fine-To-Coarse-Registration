...
if scale == 1
  xyz = depth2XYZcamera(data.K, depthRead(data.depth{frameID}, bit_shift), data.depth_resolution, 0.5 );%readDepth(frames, frame);
else
  xyz = depth2XYZcamera(data.K, depthRead(data.depth{frameID}, bit_shift), data.depth_resolution, 0.5 );
